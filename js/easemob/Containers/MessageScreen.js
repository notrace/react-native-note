    ...
    if(navigator) {
      navigator.push({
        ...
        params: {
          ...
          url: this.props.company.show_url,
...
// ------------ renders -------------
  ...
  _renderRightTxt(rowData = {}) {
    ...
    var avatar = user.userData.data.person.avatar
    return (
      <View style={[Styles.row, Styles.directionEnd]}>
        <Image source={{uri:avatar}} resizeMode='cover' style={[Styles.rowLogo, Styles.rowLogoRight]}/>
  ...
  _renderLeftTxt(rowData = {}) {
    return (
      <View style={Styles.row}>
        <Image source={{uri:this.props.ToUser.avatar}} resizeMode='cover' style={Styles.rowLogo}/>
        <View style={Styles.rowMessage}>
  ...
  _renderLeftImg(rowData = {}) {
    ...
    return (
      <View style={Styles.row}>
        <Image source={{uri:this.props.toAvatar}} resizeMode='cover' style={Styles.rowLogo}/>
        <View style={Styles.rowMessage}>
          <Text style={Styles.nameText}>{rowData.from}</Text>
...
// ------------ render -------------
  render() {
    ...
    if(this.props.company.name){
      var header =ViewUtils.getMessageNavBar(()=>this.onBack(), this.props.company.name, '查看项目',()=>this.showCompany())
    }else{
      var header =ViewUtils.getMessageNavBar(()=>this.onBack(), this.props.company.name, '',()=>{})
    }
     
    return (
      <View style={[Styles.container, costomStyles.settingContainer]}>
        {header}
        <BaseListView
          autoScroll={true}
          data={messages}
...
const costomStyles = StyleSheet.create({
  settingContainer:{
    flex: 1, 
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    sendImgMessage: (chatType, id, message, source, username) => dispatch(MessageActions.sendImgMessage(chatType, id, message, source, username)),
  }
}
